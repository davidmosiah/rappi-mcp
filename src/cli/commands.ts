...
import { TOOL_CALLS } from "../tools/rappi-tools.js";
...
async function runSetup(args: string[]): Promise<number> {
  ...
  console.log("Next: rappi-mcp-unofficial auth --token <personal-access-token>");
  return 0;
}
...
async function runDoctor(args: string[]): Promise<number> {
  ...
  if (args.includes("--json")) {
    ...
    console.log(`Rappi MCP · Doctor  ${status.ok ? "READY" : "NEEDS AUTH"}`);
...
async function runCall(args: string[]): Promise<number> {
  ...
  if (!name || name.startsWith("-")) {
    console.error("Usage: rappi-mcp-unofficial call <tool> [--json '{...}']");
...
function printHelp(): void {
  console.log(`rappi-mcp-unofficial ${SERVER_VERSION}
Unofficial local-first Rappi MCP. Never pays unless RAPPI_ALLOW_MUTATIONS and explicit_user_intent.
