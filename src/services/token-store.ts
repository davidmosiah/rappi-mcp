...
import type { RappiTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<RappiTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as RappiTokenSet;
    } catch (error) {
  ...
  async write(tokens: RappiTokenSet): Promise<void> {
